		...
		if noChildNodes {
			...
			if _, isTyped := c.typedPathsIndex[fullPath]; isTyped {
				...
				if mapValueType.Kind() == reflect.Interface {
					...
					if dyn, ok := value.(col.Dynamic); ok {
						value = dyn.Any()
					}
...
func structToJSON(v any) (*col.JSON, error) {
	json := col.NewJSON()
...
func mapToJSON(v any) (*col.JSON, error) {
	json := col.NewJSON()
	val := reflect.ValueOf(v)
...
func iterateStruct(val reflect.Value, prefix string, json *col.JSON) error {
	typ := val.Type()
...
func handleValue(val reflect.Value, path string, json *col.JSON, forcedType string) error {
	...
	case reflect.Map:
		if forcedType == "" && val.Type().Elem().Kind() == reflect.Interface {
			...
			json.SetValueAtPath(path, col.NewDynamicWithType(val.Interface(), forcedType))
			return nil
		}
...
func iterateMap(val reflect.Value, prefix string, json *col.JSON) error {
